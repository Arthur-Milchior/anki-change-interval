...
def getPercentWithResponse():
    ...
    if s is None:
        (s, r) = getText("By which percent do you want to increase/decrease percent for your card. (50% means half, 200% means double...)")
    else:
...
def decreaseDelay(cids):
    ...
    for cid in cids:
        ...
        if card.type !=QUEUE_REV:
            continue
        curentIvl = card.ivl if getUserOption("Consider last review date", False) else getattr(card, whichDue) - card.col.sched.today
